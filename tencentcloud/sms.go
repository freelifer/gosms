...
func init() {
	gosms.Register("tencent", &SMSClient{})
}
...
func (client *SMSClient) SendSMS(req *gosms.Request) (*gosms.Response, error) {
	...
	c, _ := sms.NewClient(credential, client.region, cpf)
	...
	/* 模板参数: 若无模板参数，则设置为空*/
	request.TemplateParamSet = client.templateParamSet(req)
	/* 模板 ID: 必须填写已审核通过的模板 ID。模板ID可登录 [短信控制台] 查看 */
	request.TemplateId = common.StringPtr(req.TemplateCode)
	/* 下发手机号码，采用 E.164 标准，+[国家或地区码][手机号]
	 * 示例如：+8613711112222， 其中前面有一个+号 ，86为国家码，13711112222为手机号，最多不要超过200个手机号*/
	request.PhoneNumberSet = common.StringPtrs([]string{"+86" + req.Number})
	...
	resp.Source = "tencent"
	resp.RequestId = *response.Response.RequestId
	resp.Code = *sendStatus.Code
	resp.Message = *sendStatus.Message
	resp.BizId = *sendStatus.SerialNo
	return resp, nil
}
...
func (client *SMSClient) templateParamSet(req *gosms.Request) []*string {
	...
	ptrs := make([]*string, len(req.TemplateParam))
	for i := 0; i < len(req.TemplateParam); i++ {
		ptrs[i] = &req.TemplateParam[i].Value
	}
	return ptrs
}
